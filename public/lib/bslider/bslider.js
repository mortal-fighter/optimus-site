var bslider = {
	...
	// Init Bslider and each defined slider on the webpage
	initBsliders: function () {
		'use strict';

		// Get each slider from the webpage and process it
		var sliders = document.querySelectorAll('.bslider');
		for (let i = 0; i < sliders.length; i++) {
			...
			var sliderObject = {};
			var sliderIndex = bslider.sliders.push(sliderObject) - 1;

			sliderObject.sliderContainer = sliders[i];
			sliderObject.muted = false;
			sliderObject.currentIndex = 0;
			...
			if (! currentSlider.hasAttribute('data-hide-mute') &&
				...
				muteButton.setAttribute('class', bslider.muteButtonClass);
				muteButton.setAttribute('style', bslider.muteIconStyle);
				muteButton.setAttribute('data-state', 100);
				muteButton.setAttribute('onclick', 'bslider.toggleAudio(' + sliderIndex + ')');
				var muteIcon = document.createElement('img');
				muteIcon.setAttribute('src', bslider.audioLoudIcon);
			...
			if (! currentSlider.hasAttribute('data-hide-pause') &&
				...
				pauseButton.setAttribute('style', bslider.playPauseIconStyle);
				pauseButton.setAttribute('class', bslider.pauseButtonClass);
				pauseButton.setAttribute('data-state', 'play');
				pauseButton.setAttribute('onclick', 'bslider.toggleState(' + sliderIndex + ')');
				var pauseIcon = document.createElement('img');
				pauseIcon.setAttribute('src', bslider.pauseIcon);
			...
			var slides = currentSlider.querySelectorAll('.bslide');
			sliderObject.countSlides = slides.length;

			for (var j = 0; j < slides.length; j++) {
				var slide = slides[j];

				// Set classes to hide the slide and preload audio if specified
				slide.setAttribute('style', bslider.slideFade + ";" + bslider.slideFadeOut);
			...
			// Bullets
			if (!currentSlider.hasAttribute('data-hide-bullets')) {
				
				var ul = document.createElement('ul');
				ul.setAttribute('class', 'bslider-bullets');
				...
				for (let j = 0; j < slides.length; j++) {
					...
					if (window.addEventListener) {
						li.addEventListener('click', function() { bslider.gotoSlide(j, i) }, false);
					} else if (window.attachEvent) { // IE
						window.attachEvent('onclick', function() { bslider.gotoSlide(j, i) });
					}   
					ul.appendChild(li); 
				}
				currentSlider.appendChild(ul);
			}
			...
			if (slides.length > 0) { // Don't crap out if no slides specified
				...
				slides[0].setAttribute('style', bslider.slideFade + ";" + bslider.slideFadeIn);
				sliderObject.timeoutHandle = setTimeout(function (sliderIndex, slides) {
					bslider.advanceSlide(sliderIndex);
				}, parseInt(duration) * 1000, sliderIndex, slides);
				sliderObject.currentSlide = slides[0];

				bslider._playAudio(sliderIndex);
			}
		}
	},

	gotoSlide: function(slideIndex, sliderIndex) {
		'use strict';
		...
		// Bullets
		bslider._moveBulletToPosition(slideIndex, sliderIndex);

		// Slide change animation
		slider.currentSlide.setAttribute('style', bslider.slideFade + ";" + bslider.slideFadeOut);
		slides[slideIndex].setAttribute('style', bslider.slideFade + ";" + bslider.slideFadeIn);

		// Audio start/stop + change current slide
		bslider._pauseAudio(sliderIndex);
		slider.currentSlide = slides[slideIndex];
		slider.currentIndex = slideIndex;
		bslider._playAudio(sliderIndex);

		// Timeouts
		clearTimeout(slider.timeoutHandle);
		slider.timeoutHandle = setTimeout(function () {
			bslider.advanceSlide(sliderIndex);
		}, parseInt(duration) * 1000);

	},

	_moveBulletToPosition: function(position, sliderIndex) {
		var posA = bslider.sliders[sliderIndex].currentIndex;
		var posB = position;
		var bullets = bslider.sliders[sliderIndex].sliderContainer.querySelectorAll('ul.bslider-bullets li');
		bullets[posA].setAttribute('class', '');
		bullets[posB].setAttribute('class', 'current');
	},

	advanceSlide: function (sliderIndex) {
		'use strict';
		...
		bslider.gotoSlide((slider.currentIndex + 1) % slider.countSlides, sliderIndex);
	},

	_playAudio: function(slideIndex) {
		// Given a slideshow, plays the audio for the current slide if present and not muted
		'use strict';
		if (bslider.sliders[slideIndex].muted === false) {
			var audio = bslider.sliders[slideIndex].currentSlide.querySelector('audio');
			if (audio) {
				audio.play();            
			}
		}
	...
	toggleAudio: function (sliderIndex) {
		...
		var slider = bslider.sliders[sliderIndex].sliderContainer;
		var muteButton = slider.querySelector('.'+bslider.muteButtonClass);

		if (bslider.sliders[sliderIndex].muted) {

			muteButton.querySelector('img').setAttribute('src', bslider.audioLoudIcon);
			muteButton.setAttribute('data-state', '100');
			bslider.sliders[sliderIndex].muted = false;
			bslider._playAudio(sliderIndex);

		} else {

			muteButton.querySelector('img').setAttribute('src', bslider.audioMuteIcon);
			muteButton.setAttribute('data-state', '0');
			bslider.sliders[sliderIndex].muted = true;
			bslider._pauseAudio(sliderIndex);
	...
	toggleState: function (sliderIndex) {
		...
		var pauseButton = slider.querySelector('.'+bslider.pauseButtonClass);

		if (pauseButton.getAttribute('data-state') === 'play') { // If the slider is playing, pause it

			pauseButton.querySelector('img').setAttribute('src', bslider.playIcon);// Change button icon from pause to play
			pauseButton.setAttribute('data-state', 'pause');
			clearTimeout(bslider.sliders[sliderIndex].timeoutHandle); // Stop advancing to next slide
			bslider._pauseAudio(sliderIndex);// pause audio

		} else { // If on the other hand the slider is paused, start playing

			pauseButton.querySelector('img').setAttribute('src', bslider.pauseIcon); // Change pause button icon to pause icon
			pauseButton.setAttribute('data-state', 'play');
			// Start advancing slides
			bslider.sliders[sliderIndex].timeoutHandle = setTimeout(function () {
				bslider.advanceSlide(bslider.sliders[sliderIndex].currentSlide, sliderIndex);
			}, parseInt(bslider.sliders[sliderIndex].currentSlide.getAttribute('data-duration')) * 1000);
			bslider._playAudio(sliderIndex);// unpause audio
	...
	stop: function() {
		'use strict';
		while (bslider.sliders.length > 0) {
			var slider = bslider.sliders.pop();
			clearTimeout(slider.timeoutHandle);
		}
	...
	init: function () {
		...
		bslider.stop();
		if (window.addEventListener) {
			window.addEventListener('load', this.initBsliders, false);
		} else if (window.attachEvent) { // IE
			window.attachEvent('onload', this.initBsliders);
		}
	},
