...
router.get('/', function(req, res, next) {
	res.redirect('/admin/prices/edit');
});
...
router.get('/edit', function(req, res, next) {
	...
	connectionPromise().then(function(connection) {
		db = connection;
		var sql = `SELECT id, html FROM prices WHERE id=1;`;
		console.log(sql);
		return db.queryAsync(sql);
	}).then(function(rows) {
		//todo: check rows to be fetched
		res.render('admin/admin_prices_edit', {
			message: '',
			messageType: '', 
			menu: menuGenerated,
			pricesOnce: rows[0] 
...
router.put('/', function(req, res, next) {
	...
	connectionPromise().then(function(connection) {
		db = connection;
		var sql = `UPDATE prices SET html = ${req.body.html} WHERE id = 1;`;
		console.log(sql);
		return db.queryAsync(sql);
	}).then(function() {
		res.json({
