...
router.post('/upload_single_photo', uploader.single('upload_single'), function(req, res, next) {
	...
	function process() {
		...
		jimp.read(req.file.path).then(function(image) {
			...
			for (var i = 0; i < convertOpts.length; i++) {
				sizesNew[i] = {};
				sizesKoeff[i] = 1;
				imgs[i] = (i == 0) ? image : image.clone();

				var indexDot = req.file.path.lastIndexOf('.');
				paths[i] = req.file.path.substr(0, indexDot) + convertOpts[i].prefix + req.file.path.substr(indexDot);
			}
			...
			for (let i = 0; i < convertOpts.length; i++) {

				if (dims.width > dims.height) {
					sizesKoeff[i] = dims.width / convertOpts[i].w;
				} else {
					sizesKoeff[i] = dims.height / convertOpts[i].h;
				}	

				if (sizesKoeff[i] > 1) {
					doResize = true;
					sizesNew[i].w = Math.trunc(dims.width / sizesKoeff[i]);
					sizesNew[i].h = Math.trunc(dims.height / sizesKoeff[i]);
				} else {
					doResize = false;
					sizesNew[i].w = dims.width;
					sizesNew[i].h = dims.height;
				}

				console.log(sizesKoeff[i], sizesNew[i].w, sizesNew[i].h);	

				promises.push((function() {
					return new Promise(function(resolve, reject) {
						try {
							if (doResize) {
								imgs[i].resize(sizesNew[i].w, sizesNew[i].h);
							}
							imgs[i].quality(convertOpts[i].quality).write(paths[i]);
							resolve();
						} catch (err) {
							reject(err);
						}
			...
			var sql = `	
				INSERT INTO info_units_photos(src, info_unit_id, width, height, date_created, type)
					VALUES 
				`;

			for (var i = 0, len = convertOpts.length; i < len; i++) {
				
				var href = convertResourceLocator(paths[i]);
				
				sql += `('${href}', ${req.body.infoUnitId}, ${sizesNew[i].w}, ${sizesNew[i].h}, NOW(), '${convertOpts[i].title}')`;
				if (i !== (len - 1)) {
					sql += ', ';
				}
			}	
			...
			returnHref = convertResourceLocator(paths[1]);
			...
			// Something bad has happened during compression, so we'll RESET ALL what we've done with this photo
			// and return bad status code
	...
	function cleanup() {
		...
		return Promise.resolve().then(function() {
			...
			if (insertId) {
				var sql = `DELETE FROM info_units_photos WHERE id = ${insertId};`;
				console.log(sql);
				return db.queryAsync(sql);
			} else {
				return Promise.resolve();
			...
			for (let i = 0; i < convertOpts.length; i++) {
				promises.push((function() {
					return new Promise (function(resolve, reject) {
						// find out if photo with path `paths[i]` exists...
						fs.stat(paths[i], function(err, stat) {
							if (err) {
								reject(err);
							};

							// ... and if so remove the photo
			...
			// dont't forget about the original picture
			promises.push((function() {
				return new Promise (function(resolve, reject) {
					fs.stat(req.file.path, function(err, stat) {
						if (err) return;
			...
			// finally send a 404 response code
			res.json({
				code: 404,
	...
	var sizesNew = new Array(convertOpts.length);
	var sizesKoeff = new Array(convertOpts.length);
	var imgs = new Array(convertOpts.length);
	var dims = new Array(convertOpts.length);
	var paths = new Array(convertOpts.length);
	var db = null;
	var returnHref = null;
	var insertId = null;
